...
names(env.stack.0k) <- paste00("bio", c("02", "04", "10", "16", "17"))
env.stack.0k

env.stack.6k <- stack(tif.6k)
names(env.stack.6k) <- paste00("bio", c("02", "04", "10", "16", "17"))
env.stack.6k

env.stack.21k <- stack(tif.21k)
names(env.stack.21k) <- paste00("bio", c("02", "04", "10", "16", "17"))
env.stack.21k
...
# extract values and coordinates for background
...
plot(env.stack.0k[[1]])
points(coords, pch = "o", cex = 1e-1)

###-----------------------------------------------------------------------------------------###

# verify maxent
jar <- paste0(system.file(package = "dismo"), "/java/maxent.jar", sep = "")
file.exists(jar)
...
for(i in 1:length(levels(po[, 1]))){ # for to each specie
  ...
  for(r in 1:5){	# number of replicas
    ...
    # 2. gower
	  ...
	  # 1.2 projection
 writeRaster(predict(env.stack.0k, Gower), paste0(AOGCM, "_Gower_0k_", id.specie, r, ".tif", sep = ""), format = "GTiff") 
 writeRaster(predict(env.stack.6k, Gower), paste0(AOGCM, "_Gower_6k_", id.specie, r, ".tif", sep = ""), format = "GTiff")
 writeRaster(predict(env.stack.21k, Gower), paste0(AOGCM, "_Gower_21k_", id.specie, r, ".tif", sep = ""), format = "GTiff") 
 # 1.3 evaluation
...
## Maha	
	...
	# 1.2 projection
 writeRaster(predict(env.stack.0k, Maha), paste0(AOGCM, "_Maha_0k_", id.specie, r, ".tif", sep = ""), format = "GTiff") 
 writeRaster(predict(env.stack.6k, Maha), paste0(AOGCM, "_Maha_6k_", id.specie, r, ".tif", sep = ""), format = "GTiff")
 writeRaster(predict(env.stack.21k, Maha), paste0(AOGCM, "_Maha_21k_", id.specie, r, ".tif", sep = ""), format = "GTiff") 
...
## Maxent	
	...
	# 1.2 projection
 writeRaster(predict(env.stack.0k, Maxent), paste0(AOGCM, "_Maxent_0k_", id.specie, r, ".tif", sep = ""), format = "GTiff") 
 writeRaster(predict(env.stack.6k, Maxent), paste0(AOGCM, "_Maxent_6k_", id.specie, r, ".tif", sep = ""), format = "GTiff")
 writeRaster(predict(env.stack.21k, Maxent), paste0(AOGCM, "_Maxent_21k_", id.specie, r, ".tif", sep = ""), format = "GTiff") 
...
## SVM	
	...
	# 1.2 projection
 writeRaster(predict(env.stack.0k, SVM), paste0(AOGCM, "_SVM_0k_", id.specie, r, ".tif", sep = ""), format = "GTiff") 
 writeRaster(predict(env.stack.6k, SVM), paste0(AOGCM, "_SVM_6k_", id.specie, r, ".tif", sep = ""), format = "GTiff")
 writeRaster(predict(env.stack.21k, SVM), paste0(AOGCM, "_SVM_21k_", id.specie, r, ".tif", sep = ""), format = "GTiff") 
 ...
 # 1.3 evaluation
	...
	eval.names <- c(eval.names, paste0(id.specie, r, sep = ""))		
} # ends for "r"
...
write.table(eval.Bioclim, paste0("zEval_", AOGCM, "_Bioclim_", id.specie, ".txt", sep = ""))
write.table(eval.Gower, paste0("zEval_", AOGCM, "_Gower_", id.specie, ".txt", sep = ""))
write.table(eval.Maha, paste0("zEval_", AOGCM, "_Maha_", id.specie, ".txt", sep = ""))
write.table(eval.Maxent, paste0("zEval_", AOGCM, "_Maxent_", id.specie, ".txt", sep = ""))
write.table(eval.SVM, paste0("zEval_", AOGCM, "_SVM_", id.specie, ".txt", sep = ""))
